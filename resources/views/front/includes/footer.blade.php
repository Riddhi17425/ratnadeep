           ...
           <div class="footer-container">
            ...
            <div class="footer-middle-grid">
                ...
                <div class="footer-box-left">
                    <!-- Top Info -->
                    <div class="mb-5 footer-desc-certs-block">
                        <p class="footer-desc">Understand the key differences between seamless and welded tubes and how to select the right option for critical applications.</p>
                        <div class="certifications mt-4">
                            <h4 class="footer-heading">Certified To:</h4>
                            <p class="cert-text">ISO 9001:2015 · ISO 14001:2015 | ISO 45001:2018 · PED · AD 2000 · NORSOK</p>
                        </div>
                    </div>
                ...
                <div class="footer-box-right">
                    <!-- Get in Touch -->
                    <div class="mb-5 footer-get-in-touch-block">
                        <h4 class="footer-heading mb-4">Get in Touch</h4>
                    ...
                    <!-- Products -->
                    <div class="mt-lg-5 pt-lg-3 footer-products-block">
            ...
            <div class="footer-bottom">
                ...
                <div class="social-links">
                    <a href="#" class="social-icon facebook">
                      ...
                      <svg class="svg-hover" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <a href="#" class="social-icon linkedin">
                     ...
                     <svg class="svg-hover" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M22.2234 0H1.77187C0.792187 0 0 0.773438 0 1.72969V22.2656C0 23.2219 0.792187 24 1.77187 24H22.2234C23.2031 24 24 23.2219 24 22.2703V1.72969C24 0.773438 23.2031 0 22.2234 0Z" fill="#0A66C2"/><path d="M7.12031 20.4516H3.55781V8.99531H7.12031V20.4516ZM5.33906 7.43438C4.19531 7.43438 3.27188 6.51094 3.27188 5.37187C3.27188 4.23281 4.19531 3.30937 5.33906 3.30937C6.47813 3.30937 7.40156 4.23281 7.40156 5.37187C7.40156 6.50625 6.47813 7.43438 5.33906 7.43438ZM20.4516 20.4516H16.8937V14.8828C16.8937 13.5562 16.8703 11.8453 15.0422 11.8453C13.1906 11.8453 12.9094 13.2937 12.9094 14.7891V20.4516H9.35625V8.99531H12.7687V10.5609H12.8156C13.2891 9.66094 14.4516 8.70938 16.1813 8.70938C19.7859 8.70938 20.4516 11.0813 20.4516 14.1656V20.4516Z" fill="white"/>
                     </svg>
                    </a>
                </div>
            </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            // Check if google translate cookie exists to set active language
            var googtrans = getCookie('googtrans');
            var currentLang = 'en';
            if(googtrans) {
                var parts = unescape(googtrans).split('/');
                if(parts.length >= 3 && parts[2] !== '') {
                    currentLang = parts[2];
                }
            }
            $('#languageDropdownNative').val(currentLang);

            // Initialize Select2 with custom flag templates
            function formatLangState(state) {
                if (!state.id) return state.text;
                var flagCode = $(state.element).data('flag');
                if (!flagCode) return state.text;
                var $state = $(
                    '<span style="display:flex; align-items:center; gap:10px;">' +
                        '<img src="https://flagcdn.com/w20/' + flagCode + '.png" style="border-radius:2px; width:20px; box-shadow: 0 0 2px rgba(0,0,0,0.2);" /> ' +
                        '<span style="font-size: 15px;">' + state.text + '</span>' +
                    '</span>'
                );
                return $state;
            }

            $('#languageDropdownNative').select2({
                minimumResultsForSearch: Infinity,
                dropdownCssClass: 'custom-select2-dropdown',
                width: '150px',
                dropdownParent: $('.language-select-wrapper'), // Fixes dropdown shaking on scroll
                templateResult: formatLangState,
                templateSelection: function(state) { 
                    if (!state.id) { return state.text; }
                    return $('<span>' + state.text + '</span>'); 
                } // Safe return to prevent Select2 crash while hiding flag in button
            });

            // Prevent Lenis smooth scroll from hijacking the dropdown wheel scroll
            $('#languageDropdownNative').on('select2:open', function (e) {
                setTimeout(function() {
                    $('.select2-results__options').attr('data-lenis-prevent', 'true');
                }, 10);
            });

            // Handle Select2 change to trigger Google Translate
            $('#languageDropdownNative').on('select2:select', function (e) {
                var langCode = e.params.data.id;
                
                if (langCode === 'en') {
                    // Google Translate doesn't have 'en' in its dropdown for an English site.
                    // The only robust way to restore original language is clearing the cookie and reloading.
                    ...
                    if (googleSelect) {
                        ...
                        // Fallback if Google Translate hasn't fully loaded
                        var checkGoogleTranslate = setInterval(function() {
                            var gSelect = document.querySelector('.goog-te-combo');
                            if (gSelect) {
                                gSelect.value = langCode;

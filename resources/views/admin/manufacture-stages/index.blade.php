        ...
        <div class="card-body">
            <table class="table table-bordered" id="manufactureStageTable">
                ...
                <tbody>
                    ...
                    <tr class="{{ $manufactureStage->trashed() ? 'table-light text-muted' : '' }}">
                        <td>{{ $key + 1 }}</td>
                        <td>
                            @if($manufactureStage->image)
                                <img src="{{ asset($manufactureStage->image) }}" alt="{{ $manufactureStage->alt_image_text }}" width="50" style="{{ $manufactureStage->trashed() ? 'opacity:0.5' : '' }}">
                            @else
                                N/A
                            @endif
                        </td>
                        ...
                        <td>
                            @if($manufactureStage->trashed())
                                ...
                                <form action="{{ route('manufacture-stages.restore', $manufactureStage->id) }}" method="POST" class="d-inline restore-form">
                                    @csrf
                                    @method('PUT')
                                ...
                                <button type="button"
                                    ...
                                    data-id="{{ $manufactureStage->id }}"
                                    data-title="{{ $manufactureStage->title }}">
                            ...
                            @else
                                ...
                                <form action="{{ route('manufacture-stages.destroy', $manufactureStage->id) }}" method="POST" class="d-inline delete-form">
                                    @csrf
                                    @method('DELETE')
...
<script>
    ...
    $(document).ready(function () {
        ...
        $(document).on('submit', '.delete-form', function (e) {
            ...
            Swal.fire({
                title: 'Delete this manufacture stage?',
        ...
        $(document).on('submit', '.restore-form', function (e) {
            ...
            Swal.fire({
                title: 'Restore this manufacture stage?',
                icon: 'question',
                showCancelButton: true,
        ...
        $(document).on('click', '.btn-force-delete-open', function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    form.attr('action', '{{ url("admin/manufacture-stages") }}/' + id + '/force-delete');
                    form.trigger('submit');
                }
            });
        });

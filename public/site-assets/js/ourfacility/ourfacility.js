$(document).ready(function () {
    ...
    var table = $('#ourfacility_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: window.APP_URLS.getOurfacilitydata,
        order: [[0, 'desc']],
        columns: [
            { data: 'id', name: 'id' },
    ...
    // delete milestone
    $(document).on('click', '.delete_ourfacility', function () {
        let id = $(this).data('id');

        var url = window.APP_URLS.deleteourfacility.replace(':id' , id);
...
function validateAndPreviewImage() {
        const input = document.getElementById("ourfacility_image");
        const file = input.files[0];
        const errorSpan = document.getElementById("span_ourfacility_image");
        const previewImg = document.getElementById("preview_ourfacility_image");
        const bannerId = document.getElementById("span_ourfacility_image_id")?.value;

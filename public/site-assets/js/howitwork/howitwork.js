$(document).ready(function () {
    ...
    var table = $('#howitwork_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: window.APP_URLS.getHowitworkdata,
        order: [[0, 'desc']],
        columns: [
            { data: 'id', name: 'id' },
    ...
    // delete milestone
    $(document).on('click', '.delete_howitwork', function () {
        let id = $(this).data('id');

        var url = window.APP_URLS.deletehowitwork.replace(':id' , id); 
...
function validateAndPreviewImage() {
        const input = document.getElementById("howitwork_image");
        const file = input.files[0];
        const errorSpan = document.getElementById("span_howitwork_image");
        const previewImg = document.getElementById("preview_howitwork_image");
        const bannerId = document.getElementById("span_howitwork_image_id")?.value;
